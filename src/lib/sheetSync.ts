...
export const DATA_POINTS_COLUMNS = [
  "id",
  "claim",
  "category",
  "belief",
...
export const EXEMPLARS_COLUMNS = [
  "id",
  "tweet_text",
  "archetype",
  "tweet_style",
...
export function shouldPushExemplars(request: Request): boolean {
  return new URL(request.url).searchParams.get("includeExemplars") === "true";
}

export async function runPushToSheet(includeExemplars = false) {
...
export async function runPullFromSheet() {
...
export async function runReconcileSheets(includeExemplars = false) {
  const pull = await runPullFromSheet();
  const push = await runPushToSheet(includeExemplars);
